...
inline TSerialCommand ParseCommandString(String Buffer)
{
...
inline TSerialCommand CreateCommandStruct(String Command, String Param = "")
{
    TSerialCommand Result;
...
inline String CmdStr(String Command, String Param)
{
    Command.toUpperCase();
...
public:
    ...
    {
        strncpy(DeviceName, _DeviceName.c_str(), sizeof(DeviceName) - 1);
...
inline void SC_ECHO(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr(WiFi.localIP().toString(), String(Commander->DeviceType)));
}

inline void SC_WHOIS(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr(String(Commander->DeviceName), String(Commander->DeviceType)));
}

inline void SC_IP(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr("IP", WiFi.localIP().toString()));
}

inline void SC_IPMASK(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr("IPMASK", WiFi.subnetMask().toString()));
}

inline void SC_IPDNS(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr("IPDNS", WiFi.dnsIP().toString()));
}

inline void SC_IPGATEWAY(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr("IPGATEWAY", WiFi.gatewayIP().toString()));
}

inline void SC_SSID(TSerialCommander *Commander, String Param)
{
    char ssid[32] = {0};
    ...
    os_memcpy(ssid, stationConf.ssid, sizeof(ssid) - 1);

    Commander->Println(CmdStr("SSID", String(ssid)));
}

inline void SC_KEY(TSerialCommander *Commander, String Param)
{
    Commander->Println(CmdStr("KEY", "REDACTED"));
}

inline void SC_SETSSID(TSerialCommander *Commander, String Param)
{
    char ssid[32] = {0};
    // char password[64];

    Param.toCharArray(ssid, sizeof(ssid));
    struct station_config stationConf;
    wifi_station_get_config(&stationConf);
...
inline void SC_SETKEY(TSerialCommander *Commander, String Param)
{
    // char ssid[32];
    char password[64] = {0};

    Param.toCharArray(password, sizeof(password));
    struct station_config stationConf;
    wifi_station_get_config(&stationConf);
...
inline void SC_PAIRING(TSerialCommander *Commander, String Param)
{
    Pairing();
}

inline void SC_REBOOT(TSerialCommander *Commander, String Param)
{
...
inline TSerialCommander::TSerialCommander(HardwareSerial *_SerialPort, unsigned long baud, int _UDP_Port = 0)
{
...
inline TSerialCommander::~TSerialCommander()
{
#if defined(ESP8266)
    if (UDP != NULL)
        delete UDP;
    ...
    TSerialCommanderStruct *Current{NULL};
    Current = Commands->GetNext(Current);
    while (Current != NULL)
    {
        TSerialCommanderStruct *Next = Commands->GetNext(Current);
        delete Current;
        Current = Next;
    }
    delete Commands;
...
inline void TSerialCommander::Idle()
{
